...
contract MentalPoker {
  ...
  mapping (uint256 => string) public encodedCards;
  uint256 constant ORDER = 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141;
  string[52] public cards = ['2c', '2d', '2h', '2s', '3c', '3d', '3h', '3s', '4c', '4d', '4h', '4s', '5c', '5d', '5h', '5s', '6c', '6d', '6h', '6s', '7c', '7d', '7h', '7s', '8c', '8d', '8h', '8s', '9c', '9d', '9h', '9s', 'Tc', 'Td', 'Th', 'Ts', 'Jc', 'Jd', 'Jh', 'Js', 'Qc', 'Qd', 'Qh', 'Qs', 'Kc', 'Kd', 'Kh', 'Ks', 'Ac', 'Ad', 'Ah', 'As'];
  
  constructor (uint256[52] memory xvalues) public {
  ...
  function revealCard(uint256 x1, uint256 y1, uint256 key1, uint256 key2) public view returns (string memory, bool) {
      ...
      (uint256 x2, uint256 y2) = pointMultiply(x1, y1, v1);
      (x2, y2) = pointMultiply(x2, y2, v2);
      string memory c = encodedCards[x2];
      if (isNil(c)) {
          return (c, false);
      } else {
          return (c, true);
